...
def update_memory_from_evaluation(state, deps):
    ...
    attempt_number = int(state.get("attempt_number", 1) or 1)
    score = int(eval_data.get("score", 0) or 0)

    repeated_target_failure = attempt_number >= 2 and not eval_data.get("targetUsed", False)
    if repeated_target_failure:
        save_weak_item(user_id, "PATTERN", target, "target not transferred repeatedly", target, "连续尝试仍未迁移目标表达", "high")

    if action == "mark_known":
        if attempt_number == 1 and score >= 90:
            save_known(user_id, "PATTERN", target, state["card_id"])
        else:
            save_review(user_id, "PATTERN", target, f"Review after retry: {target}")
        ...
        priority = "high" if attempt_number >= 2 else "medium"
        save_weak_item(user_id, "PATTERN", target, "target not transferred", target, "未迁移目标表达", priority)
    return {}
